...
// Team fetches Factorial HR teams.
type Team struct {
	client Client
}
...
func (r *Team) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	teams, err := r.client.FetchPaginated(ctx, "resources/teams/teams")
	if err != nil {
		return nil, err
	}

	// Fetch team memberships to count members and identify leads
	memberships, err := r.client.FetchPaginated(ctx, "resources/teams/team_memberships")
...
// TeamMembership fetches Factorial HR team memberships.
type TeamMembership struct {
	client Client
}

// NewTeamMembership creates a new TeamMembership resource.
func NewTeamMembership(client Client) *TeamMembership {
	return &TeamMembership{client: client}
}

// Name returns the resource type name.
func (r *TeamMembership) Name() string {
	return "factorial_team_membership"
}

// Fetch retrieves all Factorial HR team memberships.
func (r *TeamMembership) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	memberships, err := r.client.FetchPaginated(ctx, "resources/teams/team_memberships")
	if err != nil {
		return nil, err
	}
