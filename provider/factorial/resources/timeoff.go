...
// Leave fetches Factorial HR leave requests.
type Leave struct {
	client Client
}

// NewLeave creates a new Leave resource.
func NewLeave(client Client) *Leave {
	return &Leave{client: client}
}

// Name returns the resource type name.
func (r *Leave) Name() string {
	return "factorial_leave"
}

// Fetch retrieves all Factorial HR leave requests.
func (r *Leave) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	leaves, err := r.client.FetchPaginated(ctx, "resources/timeoff/leaves")
	if err != nil {
		return nil, err
	}
...
// Allowance fetches Factorial HR leave allowances.
type Allowance struct {
	client Client
}

// NewAllowance creates a new Allowance resource.
func NewAllowance(client Client) *Allowance {
	return &Allowance{client: client}
}

// Name returns the resource type name.
func (r *Allowance) Name() string {
	return "factorial_allowance"
}

// Fetch retrieves all Factorial HR leave allowances.
func (r *Allowance) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	allowances, err := r.client.FetchPaginated(ctx, "resources/timeoff/allowances")
	if err != nil {
		return nil, err
	}
