...
// Package resources provides Factorial HR resource types for security and
// compliance scanning, including employees, contracts, teams, locations,
// documents, and time-off data.
package resources
...
// Client is an interface for the Factorial API client.
type Client interface {
	FetchPaginated(ctx context.Context, path string) ([]map[string]any, error)
}

// Employee fetches Factorial HR employees.
type Employee struct {
	client Client
}

// NewEmployee creates a new Employee resource.
func NewEmployee(client Client) *Employee {
	return &Employee{client: client}
}

// Name returns the resource type name.
func (r *Employee) Name() string {
	return "factorial_employee"
}

// Fetch retrieves all Factorial HR employees.
func (r *Employee) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	employees, err := r.client.FetchPaginated(ctx, "resources/employees/employees")
	if err != nil {
		return nil, err
	}

	// Fetch teams to check team membership
	teamMembers, err := r.client.FetchPaginated(ctx, "resources/teams/team_memberships")
