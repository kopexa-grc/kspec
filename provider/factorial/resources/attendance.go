...
// Shift fetches Factorial HR attendance shifts.
type Shift struct {
	client Client
}

// NewShift creates a new Shift resource.
func NewShift(client Client) *Shift {
	return &Shift{client: client}
}

// Name returns the resource type name.
func (r *Shift) Name() string {
	return "factorial_shift"
}

// Fetch retrieves all Factorial HR shifts.
func (r *Shift) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	shifts, err := r.client.FetchPaginated(ctx, "resources/attendance/shifts")
	if err != nil {
		return nil, err
	}
