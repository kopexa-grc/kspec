...
// AdminUser fetches organization users via Admin API.
type AdminUser struct {
	client *admin.Client
	orgID  string
}

// NewAdminUser creates a new AdminUser resource.
func NewAdminUser(client *admin.Client, orgID string) *AdminUser {
	return &AdminUser{client: client, orgID: orgID}
}

// Name returns the resource type name.
func (r *AdminUser) Name() string {
	return "atlassian_admin_user"
}

// Fetch retrieves organization users.
func (r *AdminUser) Fetch(ctx context.Context, asset core.Asset) ([]core.Resource, error) {
	if r.orgID == "" {
		return nil, nil
	}
